...
class myEditor(QMainWindow):
	def __init__(self, parent = None):
		...
		### color chooser
		tb.addSeparator()
		tb.addAction(QIcon.fromTheme("preferences-color"),"insert Color", self.insertColor)
		tb.addSeparator()
		...
		self.actionTextColor = QAction(QIcon(self.pix), "insert QColor", self,
...
#		tbf.addAction(QAction(QIcon.fromTheme("ok"), "Test Button", self, triggered=self.Test))
	...
	def cursorPositionChanged(self):
		...
		self.statusBar().showMessage("line " + str(line))
...
#		self.statusBar().showMessage("found bookmark")
	...
	def contextMenuRequested(self,point):
		...
		cmenu.addSeparator()
		cmenu.addAction(QIcon.fromTheme("preferences-color"),"insert Color", self.insertColor)
		if not self.editor.textCursor().selectedText() == "":
			cmenu.addSeparator()
